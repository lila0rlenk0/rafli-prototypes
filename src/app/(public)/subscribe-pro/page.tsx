...
// Pin the Pro plan at the route level — see the matching note on
// `/subscribe-basic` for the binding rationale.
const PRO_PLAN = SUBSCRIBE_PLANS[SUBSCRIBE_PLAN_SLUGS.PRO];

/**
 * `/subscribe-pro` — Pro Access Pass marketing landing
 * ($100 → $125, 20% OFF, 25 free weekly entries, priority access, 100 tickets).
 *
 ...
 * receive `PRO_PLAN` so the funnel posts the `pro_access_pass` slug to
 ...
 *
 * @returns Marketing landing for the Pro tier
 */
export default async function SubscribeProPage() {
	...
	return (
		<>
			...
			>
				<HeroSection plan={PRO_PLAN} />
				<UrgencyCountdown payoutUsd={PRO_PLAN.payoutUsd} />
				<BenefitsSection plan={PRO_PLAN} />
				<PrizeSection
					raffles={raffles}
					ticketsPerCycle={PRO_PLAN.ticketsPerCycle}
				/>
				<CtaSection plan={PRO_PLAN} />
			</SubscribeNavbar>
			<Footer />
		</>
	);
