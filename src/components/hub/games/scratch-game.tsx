...
const CARD_W = 320;
const CARD_H = 224;
/** Credits a winning scratch pays (on top of the 1-credit stake). */
const WIN_AWARD = 1;
...
/** A lightning spark flung off the foil at a scratch point (percent coords). */
interface Spark {
	readonly id: number;
	readonly x: number;
	readonly y: number;
...
function useScratchCanvas({
	...
	// Throttle: only every Nth scratch move flings a spark, to keep the count sane.
	const moveCount = useRef(0);
	// Charge the 1-credit stake once, on the first scratch stroke — matches the
	// leave-guard copy and mirrors how coin/mystery deduct on commit.
	...
	const spawnSpark = useCallback((xPct: number, yPct: number) => {
		const id = (sparkId.current += 1);
		setSparks(prev => [
			...prev,
			{ id, x: xPct, y: yPct, rotate: -22 + Math.random() * 44 },
		]);
	...
	const handleMove = useCallback(
		(event: React.MouseEvent | React.TouchEvent) => {
			...
			setCoverage(pct);
			resolveCoverage(pct);
			// Fling a lightning spark from the scratch point every few moves.
			moveCount.current += 1;
			if (moveCount.current % 4 === 0) {
				spawnSpark((pos.x / canvas.width) * 100, (pos.y / canvas.height) * 100);
			}
		},
		[resolveCoverage, spawnSpark],
	);
		...
		<>
			{messages.map(m => {
				...
				return (
					<div
						key={m.id}
						className="hg-float-msg border-brand-dark font-clash-display text-ink-900 pointer-events-none absolute z-40 rounded-full border-2 bg-white px-3.5 py-1.5 text-base font-bold whitespace-nowrap"
						style={style}
					>
						{m.text}
					</div>
...
/**
 * Lightning bolts flung off the foil at each scratch point. Sits in an
 * unclipped overlay above the card (z-30) so the bolts read as "coming out"
 * of the surface as it's scratched.
 *
 * @returns The spark overlay
 */
function SparkLayer({ sparks }: SparkLayerProps) {
	return (
		<div className="pointer-events-none absolute inset-0 z-30">
			{sparks.map(s => {
				...
				return (
					<Zap
						key={s.id}
						className="hg-spark fill-brand-yellow text-brand-dark absolute size-6"
						style={style}
					/>
				);
			})}
...
function PlayingHero({ scratch }: PlayingHeroProps) {
	const { coverage, outcome, sparks, canvasRef, onStart, onMove, onStop } =
		scratch;
	const prizeVisible =
	...
	return (
		<>
			<div className="relative shrink-0">
				<div className="border-brand-dark relative h-56 w-80 overflow-hidden rounded-xl border-2 select-none">
					<div
...
/**
 * The daily scratch card. Opens straight into scratching — drag across the
 * canvas to erase foil; the 1-credit stake is charged on the first stroke.
 * The prize stays blurred until ~58% is cleared, then resolves to a pre-rolled
 * win (+1) or loss at ~68%. Leaving mid-scratch prompts a confirmation since
 * the credit would be forfeited.
 *
			...
			<GameCta>
				{phase === 'playing' ? (
					<Button
						type="button"
						size="sm"
						className="hg-wiggle border-brand-dark self-center rounded-full"
						variant="outline"
						disabled
					>
