...
import { WinnerChatActions } from './actions';
import { WinningStatusStepper } from './status-stepper';
...
export function WinnerChatIntroPanel({
	...
	return (
		...
		//
		// Palette choice: `bg-role-host-bg/50` ties the panel to the brand's
		// warm cream accent without the full-intensity promo surfaces
		// (`bg-brand-yellow`) used on browse. The chat surface should read
		...
		<section
			...
			className="bg-role-host-bg/50 w-full rounded-2xl border border-black/10"
		>
			<Collapsible open={open} onOpenChange={setOpen} className="flex flex-col">
				...
				<CollapsibleTrigger
					className={cn(
						...
						'hover:bg-role-host-bg/70 active:bg-role-host-bg/80',
					)}
				>
					{/* Icon pill echoes the brand's "pill + hairline black
					    ...
					    from. */}
					<span className="bg-brand-yellow flex size-10 shrink-0 items-center justify-center rounded-full border border-black/10 text-black">
						<Package aria-hidden="true" className="size-4.5" />
					</span>
					<div className="flex min-w-0 flex-1 flex-col gap-1">
						<h3
							id={PANEL_TITLE_ID}
							className="font-clash-display text-foreground text-base/tight font-semibold tracking-tight"
						>
							Prize coordination
						</h3>
						<p className="text-foreground/70 text-xs/snug">
