...
import { useModeSwitcher } from './use-switcher';
...
export function ModeSwitchToggle() {
	...
	if (state.phase === 'loading') {
		return (
			<div
				className="flex h-9.5 w-full items-center overflow-hidden rounded-full border border-black opacity-50"
	...
	if (state.phase === 'promote') {
		return (
			<Link
				href="/verification"
				className="flex h-9.5 w-full items-center justify-center rounded-full bg-black text-sm font-semibold text-white"
	...
	return (
		<div
			className="flex h-9.5 w-full overflow-hidden rounded-full border border-black"
			data-testid="mode-switch-toggle"
		>
			<button
				onClick={onHostClick}
