...
export function buildConfirmingPhases({
	...
	function getConfirmationStatus(): ConfirmingPhase['status'] {
		if (!isBroadcast) return 'pending';
		if (reachedTarget) return 'done';
		return 'active';
	}

	function getVerifyingStatus(): ConfirmingPhase['status'] {
		if (!reachedTarget) return 'pending';
		if (finalizationRequested) return 'done';
		return 'active';
	}
