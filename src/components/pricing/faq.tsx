			...
			>
				{FAQ_ITEMS.map((item, index) => (
					<AccordionItem
						...
						key={item.question}
						value={`faq-${index}`}
						className="rounded-3xl border-none bg-sky-100 px-6 py-2 sm:px-8"
					>
						<AccordionTrigger className="py-6 text-left text-base font-semibold text-black hover:no-underline sm:text-lg">
							{item.question}
						</AccordionTrigger>
						<AccordionContent className="max-w-column-md pt-0 pb-6 text-sm font-medium text-black sm:text-base">
							{item.answer}
						</AccordionContent>
					</AccordionItem>
				))}
