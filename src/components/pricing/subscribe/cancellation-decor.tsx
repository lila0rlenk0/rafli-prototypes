...
/**
 * Cancellation modal decor — two mirrored "lottery card" clusters bleeding
 * off the top-left and top-right corners of the confirm dialog.
 *
 * Faithful to Figma's `imgMaskGroup` export: each cluster is a red square,
 * a pink slab, and a yellow tile drawn with the same path commands the
 * designer baked into the source SVG. Path data is preserved verbatim
 * (rather than re-derived as rotated rounded rectangles) because the
 * pink/yellow shapes carry subtle skew that won't reproduce cleanly with
 * `rotate-*` utilities — the crops at the modal edges depend on the exact
 * geometry to land where the designer intends.
 *
 ...
 * mid-animation, two copies could share `mask0_0_13` and the second one
 ...
 */
export function CancellationDecor() {
	...
	return (
		...
		>
			...
			<g mask={`url(#${maskId})`}>
				{/* Right-corner cluster — back-to-front: red square, pink
				    slab, yellow tile. Coordinates copied from the Figma
				    asset; do not "simplify" to rotate utilities — the pink
				    and yellow shapes are non-uniformly scaled and would
				    drift against the modal edge if recomputed. */}
				<g>
					<path
