...
interface CancelSubscriptionDialogProps {
	...
	/**
	 * Period-end date snapshot from `subscription.currentPeriodEnd`. Threaded
	 * in so the confirm body can state the exact retention window up-front
	 * ("keep <plan> until <date>") rather than waiting for the success
	 * body to surface it after the mutation. Optional because not every
	 * caller has the date in scope when opening the dialog.
	 */
...
/**
 * Self-serve cancellation dialog opened from the profile subscription card.
 *
 * Relocated from `src/components/pricing/subscribe/` once subscription
 * management moved off the pricing page. Copy emphasizes the period-end
 * retention window ("keep <plan> until <date>. No further charges.") so
 * users don't conflate "cancel" with "lose access immediately".
 *
 ...
 * `expiresAt` so the user re-reads when their benefits actually lapse.
 *
...
function ConfirmBody({
	...
	const periodEndCopy = currentPeriodEnd ? formatDate(currentPeriodEnd) : null;
				...
				<DialogDescription className={DESCRIPTION_CLASS}>
					{periodEndCopy ? (
						<>
							...
							<span className="font-semibold">{planName}</span> access until{' '}
							{periodEndCopy}. No further charges. After that, you&rsquo;ll give
							up:
						</>
					) : (
						<>
		...
		<div className="flex flex-col items-center gap-10 p-2 text-center sm:px-4">
			...
			<DialogHeader className="gap-4 text-center sm:text-center">
				...
				<DialogDescription className={DESCRIPTION_CLASS}>
					You&rsquo;ll keep your{' '}
					<span className="font-semibold">{planName}</span> access until{' '}
					{formattedExpiresAt}. No further charges. After that you&rsquo;ll lose
					your free weekly entries, subscriber-only pools, and entry discounts.
					<br />
