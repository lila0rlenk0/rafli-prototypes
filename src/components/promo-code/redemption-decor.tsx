...
/**
 * Credit-code redemption modal decor — two mirrored "lottery card"
 * clusters bleeding off the top-left and top-right corners of the
 * success dialog.
 *
 * Geometry shared verbatim with `CancellationDecor`: the underlying card
 * silhouettes (back square, mid slab, front tile) carry subtle non-uniform
 * skews that won't reproduce cleanly with `rotate-*` utilities, so the
 * Figma path data is preserved as-is. Only the fill palette changes — sky
 * (back) → mint (middle) → yellow (front) — to give the celebratory
 * surface a softer, on-brand colour story instead of the cancellation
 * flow's red/pink/yellow loss-framing.
 *
 ...
 *
 * Why hex literals (not Tailwind tokens):
 * SVG `fill` cannot reference Tailwind colour tokens directly. Each hex
 * is pinned to its `--color-brand-*` token in `src/app/globals.css` —
 * sky / mint / yellow are the DESIGN.md "accent trio always used as a
 * set" for modal success surfaces. If a token moves, update the literal
 * here in lock-step.
 *
	...
	return (
		...
		>
			<mask
				...
				// property via Tailwind's arbitrary-property syntax.
			...
			<g mask={`url(#${maskId})`}>
				{/* Right-corner cluster — back-to-front: sky square, mint
				    slab, yellow tile. */}
				<g>
					<path
