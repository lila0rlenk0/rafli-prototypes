...
import { RaffleMediaCarousel } from '@/components/raffle/media/media-carousel';
...
const RaffleMediaLightbox = dynamic(
	() =>
		import('@/components/raffle/media/media-lightbox').then(
...
function GalleryThumbnail({ url, index, onClick }: GalleryThumbnailProps) {
	...
	return (
		<button
			...
			className="focus-visible:ring-primary border-ink-200 relative flex aspect-square w-full min-w-0 cursor-pointer items-center justify-center overflow-hidden rounded-lg border bg-white transition-opacity hover:opacity-80 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none sm:max-h-32"
			aria-label={
