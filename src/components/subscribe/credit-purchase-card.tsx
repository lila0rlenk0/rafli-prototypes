 ...
 *
 * Module-level constant (not memoized in component) because the
 * palette is static — no prop or state drives any field today, so
 * holding the reference outside the render loop is the cheapest way to
 * guarantee `<CheckoutProvider>` never re-inits the iframe just because
...
function EmbeddedCheckout({ session }: EmbeddedCheckoutProps) {
	...
	// `FANBASIS_THEME` is a module-level constant so it does not need
	// to live in the dep list.
	const config = useMemo<CheckoutConfig>(
		() => ({
