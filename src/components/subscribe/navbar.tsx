...
import { TrustpilotBadge } from '@/components/ui-custom/trustpilot-badge';
 ...
 * sign-in CTA) down to two elements: brand mark on the left, Trustpilot
 * trust signal on the right. The /subscribe page is a conversion
 * surface — every extra nav affordance is a leak away from the
 * credit-purchase CTA, so the shell stays deliberately empty.
 *
	...
	return (
		<>
			<nav className="bg-background border-ink-900 relative z-(--z-sticky-hi) border-b">
				<div className="max-w-hero mx-auto flex h-14 w-full items-center justify-between gap-3 px-6 sm:h-16 md:px-(--spacing-navbar-pad)">
					<Link href="/" aria-label="Rafli home">
						<Logo className="h-5 w-auto sm:h-6" />
					</Link>

					<TrustpilotBadge className="shrink-0" />
				</div>
			</nav>
