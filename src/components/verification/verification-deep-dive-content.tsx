...
export interface VerificationDeepDiveContentProps {
	data: RaffleVerificationPayload;
}

export function VerificationDeepDiveContent({
	data,
}: VerificationDeepDiveContentProps) {
	return (
		...
		<div className="rounded-2xl border border-black bg-white p-6">
			...
			<div className="flex flex-col gap-4">
				<ProofLink
					...
					label="IPFS Manifest"
					description="Complete entry data, immutable"
				...
				<ProofLink
					...
					description="Entry data locked before draw"
			...
			>
				<div className="flex items-center gap-3">
					<div className="flex size-8 items-center justify-center rounded-full bg-amber-100 text-sm font-semibold text-amber-700">
						{/* Backend 0-indexed; +1 for human display */}
						{winner.position + 1}
					</div>
					<div className="text-left">
						<div className="text-sm font-medium">
							Entry #{winner.actualTicketId}
						</div>
		...
		>
			<AccordionItem value="notes" className="border-none">
				...
				<AccordionContent className="flex flex-col gap-4 px-6 pb-6">
					...
					<ul className="flex list-inside list-disc flex-col gap-1 text-sm text-neutral-600">
						<li>
							Random number comes from Chainlink VRF on Arbitrum blockchain
						</li>
						<li>
							Entry data is committed to IPFS before random number generation
						</li>
						<li>Merkle tree verifies each entry was in the committed set</li>
						<li>
					...
					>
						Learn more about provably fair sweepstakes
						<ExternalLink className="size-3" />
					</Link>
				</AccordionContent>
			</AccordionItem>
