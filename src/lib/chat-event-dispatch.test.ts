...
describe('dispatchChatEvent', () => {
	...
	describe('ack / error / presence / read_receipt', () => {
		test('ack resolves the pending send by tempId', () => {
			...
			dispatchChatEvent(
				makeEvent({ type: 'ack', tempId: 'temp-1' }),
				store,
				client,
				typingTimers,
			);
		...
		test('error without tempId is a no-op on pending sends', () => {
			...
			dispatchChatEvent(
				makeEvent({ type: 'error', code: 'rate_limited' }),
				store,
				client,
				typingTimers,
			);
		...
		test('error with unknown-format tempId defaults the code when absent', () => {
			...
			dispatchChatEvent(
				makeEvent({ type: 'error', tempId: 'temp-2' }),
				store,
				client,
				typingTimers,
			);
		...
		test('presence event without userId is a no-op', () => {
			...
			dispatchChatEvent(
				makeEvent({ type: 'presence', online: true }),
				store,
				client,
				typingTimers,
			);
	...
	// Kept here (not the chat-utils suite) because the decision depends on
