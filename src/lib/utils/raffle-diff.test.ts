...
describe('computeRaffleDiff', () => {
	describe('no changes', () => {
		test('returns empty object when nothing changed', () => {
			const diff = computeRaffleDiff(BASE_RAFFLE, BASE_FORM, 'cat-1', 'q-1');
			expect(diff).toEqual({});
		});
	});
	...
	describe('scalar text fields', () => {
		test('detects title change', () => {
			...
			const diff = computeRaffleDiff(BASE_RAFFLE, form, 'cat-1', 'q-1');
	...
	describe('category and question', () => {
		test('detects category change', () => {
			const diff = computeRaffleDiff(BASE_RAFFLE, BASE_FORM, 'cat-2', 'q-1');
			expect(diff.categoryId).toBe('cat-2');
		});

		test('detects question change', () => {
			const diff = computeRaffleDiff(BASE_RAFFLE, BASE_FORM, 'cat-1', 'q-2');
			expect(diff.questionId).toBe('q-2');
		});

		test('handles null questionId in original', () => {
			const raffle = { ...BASE_RAFFLE, questionId: null };
			const diff = computeRaffleDiff(raffle, BASE_FORM, 'cat-1', 'q-1');
		...
		test('no question diff when both empty', () => {
			const raffle = { ...BASE_RAFFLE, questionId: null };
			const diff = computeRaffleDiff(raffle, BASE_FORM, 'cat-1', '');
	...
	describe('crypto fields', () => {
		...
		test('ignores array order for chain IDs comparison', () => {
			...
			const diff = computeRaffleDiff(raffle, form, 'cat-1', 'q-1');
...
describe('hasRaffleChanges', () => {
	test('returns false when no changes', () => {
		expect(hasRaffleChanges(BASE_RAFFLE, BASE_FORM, 'cat-1', 'q-1')).toBe(
			false,
		);
	});
	...
	test('returns true when title changed', () => {
		...
		expect(hasRaffleChanges(BASE_RAFFLE, form, 'cat-1', 'q-1')).toBe(true);
	});

	test('returns true when only category changed', () => {
		expect(hasRaffleChanges(BASE_RAFFLE, BASE_FORM, 'cat-99', 'q-1')).toBe(
			true,
		);
	});
});
