 ...
 *
 * Flow:
 *   1. POST `/payments/fanbasis/public-credit-checkout` with an empty
 *      body — the embedded Fanbasis iframe collects email, card, and
 *      terms acceptance internally, so the session-mint endpoint
 *      receives no client-supplied data.
 *   2. Validate response shape; surface drift to Sentry.
 *   3. Forward the embed config to the client component, which mounts
 ...
 */
export async function createFanbasisPublicCreditCheckout(): Promise<
	ServiceResponse<
